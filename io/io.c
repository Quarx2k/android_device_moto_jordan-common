 ...
 *
 * adapted for Android by tanguy.pruvot@gmail.com
 *
 */
...
{
	fprintf(stderr,
"Raw memory i/o utility - $Revision: 2.0a $\n\n"
...
main (int argc, char **argv)
{
	...
	if (filename && (memfunc == MEM_READ)) {
		int n = write(ffd, ADDPTR(real_io,offset), req_len);
		...
		int n = read(ffd, ADDPTR(real_io,offset), req_len);
		...
		{
		case MEM_READ:
			memread_memory(req_addr, ADDPTR(real_io,offset), req_len, iosize);
			break;
		case MEM_WRITE:
			write_memory(req_addr, ADDPTR(real_io,offset), req_len, iosize, req_value);
			break;
		case MEM_AND:
			and_write_memory(req_addr, ADDPTR(real_io,offset), req_len, iosize, req_value);
			break;
		case MEM_OR:
			or_write_memory(req_addr, ADDPTR(real_io,offset), req_len, iosize, req_value);
			break;
		}
	}
